                    ...
                    .map(([score, names]) => (
                      <div key={score} className="flex items-start justify-between gap-3 py-2.5">
                        <div className="flex flex-wrap gap-1 flex-1">
                          {names.sort().map(name => (
                            <span key={name} title={name} className="text-xs bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded px-1.5 py-0.5">{name.substring(0, 4)}</span>
                          ))}
                        </div>
