  ...
  const navLinks = (mobile: boolean) => (
    <>
      ...
      <a
        href="/palpites"
        onClick={() => setOpen(false)}
        className={`hover:text-yellow-300 transition-colors text-white font-semibold tracking-wide uppercase ${mobile ? 'py-2 text-base' : 'text-sm'} ${pathname.startsWith('/palpites') ? 'text-yellow-300' : ''}`}
      >
        Palpites
      </a>
      <a
